class Api::V1::ChecklistItemsController < Api::V1::BaseController
  def index
    ...
    if items.empty?
      initialize_checklist_for(track)
      items = current_user.checklist_items.where(track: track).order(month: :asc, id: :asc)
    end

    render json: { checklist_items: items }
  end

  def toggle
    item = current_user.checklist_items.find(params[:id])
    item.update!(completed: !item.completed)
    render json: { checklist_item: item }
  end

  def bulk_create
    track = params[:track] || "SWE"
    initialize_checklist_for(track)
    items = current_user.checklist_items.where(track: track).order(month: :asc, id: :asc)
    render json: { checklist_items: items }
  end
  ...
  def initialize_checklist_for(track)
    if track == "SWE"
      items_data = [
      ...
      items_data = [
        ...
        { month: 6, week_label: "M6", text: "10 STAR behavioral stories + 2 signature designs fluent" }
      ]
    end

    items_data.each do |item|
      current_user.checklist_items.find_or_create_by!(
        track: track,
        month: item[:month],
